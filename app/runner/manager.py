...
def start_engine(mode: str = 'paper', interval: int = 10, amount: float = 1.0) -> None:
    ...
    if _engine_instance and _engine_thread and _engine_thread.is_alive():
        ...
        return
    _engine_instance = TradingEngine(mode=mode, poll_interval=interval, order_amount=amount, handle_signals=False)
    _engine_thread = threading.Thread(target=_engine_instance.start, daemon=True)
